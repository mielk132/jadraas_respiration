...
#libraries
library(mgcv); library(nlme); library(ggplot2); library(multcomp); library(car); library(ape);library(readxl);library(decimaldate);library(MASS);library(grid);library(readxl);library(ggplot2)
library(ggsci);library(tidyr);library(tidyverse);library(scales);library(gridExtra);library(grid);library(lattice);library(MASS);library(lme4);library(nlme);library(emmeans);library(gamm4);library(dplyr);library(magrittr);library(reshape2);library(mgcv);library(lmerTest);library(lubridate);library(car);library(lmerTest);library(pls); library(forcats);library(patchwork)
...
data.gs <- read.csv("/Users/lske0002/Projects/mycorrhizal removal/Response Curves/data/Tsoil&VWC_fourdayavg_noNov_13JULY2020_missingLoggers.csv") 
...
#colors for DC, 
cbp2 <- c("black", "#E6CF02", "#009E73","#56B4E9","#E69F00")
...
######### Making a Time Series of each plot for VWC and Temp ######## ##########

#summary of jädraås climate data by year and Treatment from daily averages
env_data_growingseason_bydate <- env.data.growingseason %>%
  group_by(Treatment,Block,Date) %>%
  drop_na() %>% 
  summarize_at(c("VWC","Tsoil"), funs(mean, min, max,sd, n()))
env_data_growingseason_bydate

env_data_growingseason_bydate$plot <- as.factor(paste(env_data_growingseason_bydate$Treatment, env_data_growingseason_bydate$Block ,sep = ""))

#making a numeric indicator for each plot
env_data_growingseason_bydate$plot_num <- as.numeric(as.factor(paste(env_data_growingseason_bydate$Treatment, env_data_growingseason_bydate$Block ,sep = "")))


plot(new_gs_data$Tsoil_mean, new_gs_data$Date, 
     
     
     x <- subset(env_data_growingseason_bydate, plot %in% c("S1"))
     
     par(mfrow = c(1,1))  # Set up a 2 x 2 plotting space
     
     # Create the loop.vector (all the plots)
     
     for (i in unique(env_data_growingseason_bydate$plot))  { # Loop over all plots
       
       # store data in plot i as x
       d <- subset(env_data_growingseason_bydate, plot %in% c(i))
       
       # Plot  line of d
       plot(d$Date,d$Tsoil_mean,
            main = paste("Plot", i),
            xlab = "Date",
            ylab = "Temperature (C)",
            ylim = c(0, 30), 
            xlim = as.Date(c("2017-05-01","2019-10-31")))
     }
...
env_data_gs_byplot_block5_S <- env_data_gs_byplot_block5[env_data_gs_byplot_block5$Treatment == "S",]
env_data_gs_byplot_blocks257_noS <- env_data_gs_byplot_blocks257[env_data_gs_byplot_blocks257$Treatment %in% c("SPE","SP","SE","DC"),]

env_data_gs_17 <- subset(rbind(env_data_gs_byplot_block5_S,env_data_gs_byplot_blocks257_noS),year %in% c("17"))
...
#write.csv(rbind(anova.t18,anova.t19,anova.vwc18,anova.vwc19),"anova_gs_soilt&vwc_output.csv")


#The effects of year and treatments are difficult to explain soil moisture because of interactions, so I will look at each year individually with a post hoc test without DC

env_data_gs_17 <- subset(env_data_growingseason_new_noDC, year %in% c("17"))
env_data_gs_18 <- subset(env_data_growingseason_new, year %in% c("18"))
env_data_gs_19 <- subset(env_data_growingseason_new, year %in% c("19"))

lme.vwc.17 <-lme(VWC_mean ~ Treatment,
              random = ~ 1|plot/Block,
              data = na.omit(env_data_gs_17))
anova(lme.vwc.17)
plot(resid(lme.vwc.17))
emmeans.vwc.17 <- emmeans(lme.vwc.17, c("Treatment"), type = "response")
pairs(emmeans.vwc.17, type = "response", adjust = "bonf")
